...
void cleaningSineTracks(std::vector< std::vector<Real> >&freqsTotal, const int minFrames){
  ...
  int nFrames = (int)freqsTotal.size();
  int begTrack = 0;
  if (nFrames > 0 )
  {
    int f = 0;
    int nTracks = (int)freqsTotal[0].size(); // we assume all frames have a fix number of tracks
...
{
	int n_peaks = (int)iploc.size(); // num of peaks

	int ii=0,jj=0, ploc_int;

	// get FFT size from input vector
	int size_spec_half = (int)outfft.size();
