...
"\n"
"References:\n"
"  [1] Multiplexer - Wikipedia, the free encyclopedia, http://en.wikipedia.org/wiki/Multiplexer\n");
...
void Multiplexer::compute() {
    ...
    int size = 0;
    int nFrames = 0;
    if (int(_realInputs.size())) nFrames = (int)_realInputs[0]->get().size();
    else if (int(_vectorRealInputs.size())) nFrames = (int)_vectorRealInputs[0]->get().size();
    ...
    for (int i=0; i<int(_vectorRealInputs.size()); i++) {
      ...
      for (int j=0; j<nFrames;++j) {
        if (int(frame[j].size()) > maxVecSize) maxVecSize = (int)frame.size();
      }
      size += maxVecSize;
      //size += frame[0].size();
    }
