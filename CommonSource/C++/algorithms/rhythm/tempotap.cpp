...
"\n"
"References:\n"
"  [1] F. Gouyon, \"A computational approach to rhythm description: Audio features for the computation of rhythm periodicity functions and their use\n in tempo induction and music content processing,\" UPF, Barcelona, Spain, 2005.\n\n"
"  [2] M. Davies and M. Plumbley, \"Causal tempo tracking of audio,\" in International Symposium on Music Information Retrieval (ISMIR'04), 2004.");
...
void TempoTap::configure() {
  ...
  int nbeats = (int)tempoHints.size();
  if (nbeats > 2) {
...
void TempoTap::compute() {
  ...
  const int nframe = (int)_featuresNew.size();
...
void TempoTap::computePhases(const vector<vector<Real> >& features) {
  ...
  int nfeats = (int)features.size();
  int nframe = features.empty() ? 0 : (int)features[0].size();
