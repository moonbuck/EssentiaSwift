...
void NoveltyCurveFixedBpmEstimator::compute() {
  ...
  Real threshold = mainPeaksMean(peaksPositions, peaksAmplitudes, (int)maCorr.size());
  while (true) {
    range = (int)maCorr.size();
    ...
    int nPeaks = (int)peaksPositions.size();
...
Real NoveltyCurveFixedBpmEstimator::mainPeaksMean(const vector<Real>& positions,
  ...
  nPeaks = (int)peaksPositions.size();
