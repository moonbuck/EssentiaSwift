...
"\n"
"References:\n"
"  [1] Smith, J.O.  Introduction to Digital Filters with Audio Applications, http://ccrma-www.stanford.edu/~jos/filters/\n\n"
"  [2] Infinite Impulse Response - Wikipedia, the free encyclopedia, http://en.wikipedia.org/wiki/IIR");
  ...
  if (_b.size() == _a.size()) {
    ...
    default:
      for (int n=0; n < int(y.size()); ++n) {
        ...
        updateStateLine(_state, (int)_state.size(), _a, _b, x[n], y[n]);
      }
    }
  }
    ...
    for (int n=0; n < int(y.size()); ++n) {
      y[n] = _b[0]*x[n]  + _state[0];
      updateStateLine(_state, (int)_a.size(), _a, _b, x[n], y[n]);

      for (int k=(int)_a.size(); k < int(_state.size()); ++k) {
    ...
    for (int n=0; n < int(y.size()); ++n) {
      y[n] = _b[0]*x[n]  + _state[0];
      updateStateLine(_state, (int)_b.size(), _a, _b, x[n], y[n]);

      for (int k=(int)_b.size(); k < int(_state.size()); ++k) {
