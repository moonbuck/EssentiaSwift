...
private func differenceMean<T:FloatingPoint>(_ array1: [T],
                                             _ array2: [T],
...
///   - countMismatch: Boolean flag for indicating `array1` and `array2` have differening counts.
/// - Returns: The mean of the array generated by subtracting elements in `array2` from `array1`.
private func differenceMean(_ array1: [DSPComplex],
                            _ array2: [DSPComplex],
                                            ...
                                            line: UInt = #line) -> Bool
{
  ...
  guard !equal(actual, expected, deviation: deviation) else { return true }
                                                   ...
                                                   line: UInt = #line) -> Bool
{
  ...
  guard abs(average - expected) > accuracy else  { return true }
                                                   ...
                                                   line: UInt = #line) -> Bool
{
  ...
  guard !equal(average, expected, deviation: deviation) else  { return true }
...
{
  var countMismatch = false
  let calculatedMean = differenceMean(array1, array2, countMismatch: &countMismatch)

  guard countMismatch || calculatedMean >= mean else  { return true }

  guard countMismatch ||  abs(calculatedMean - mean) > accuracy else { return true }
  ...
  where T:FloatingPoint
{
  ...
  if !countMismatch {
    ...
    for (subarray1, subarray2) in zip(array1, array2) {
      ...
      sumMean += differenceMean(subarray1, subarray2, countMismatch: &countMismatch)
    ...
    if !countMismatch {
      ...
      calculatedMean = sumMean / T(array1.count)
  ...
  where T:FloatingPoint
{
  ...
  guard countMismatch || calculatedPercentDeviation >= deviation else { return true }

  guard countMismatch || abs(calculatedPercentDeviation - deviation) > accuracy else { return true }
