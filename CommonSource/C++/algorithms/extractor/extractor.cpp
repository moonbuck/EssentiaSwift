...
void Extractor::computeRelativeIoi(Pool& p) {
  ...
  TNT::Array2D<Real> ioi_peaks((int)pos.size(), 2);
...
void Extractor::postProcessOnsetRate(VectorInput<Real>* gen, Pool& pool) {
  int nOnsets = 0;
  try {
    nOnsets = (int)pool.value<vector<Real> >(_rhythmspace + "onset_times").size();
  }
...
void Extractor::tuningSystemFeatures(Pool& pool) {
  ...
  int offset = (int)hpcp.size() - idxMax;
